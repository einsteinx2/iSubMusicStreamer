...
- [x] **[COV-11] P2 — Audio-engine integration tests against real BASS.** Drive Bass/BassPlayer/BassEqualizer with bundled fixture audio.
  > **Note (found while writing this suite):** `BassPlayer.handleInterruption`/`handleRouteChange` decode `notification.userInfo` values with `as? AVAudioSession.InterruptionType` etc., which fails for the `NSNumber` raw values the system actually sends — so real audio-session interruptions and route changes are currently ignored. The tests assert the intended behavior under strict `XCTExpectFailure` markers; fix by constructing the enums via `init(rawValue:)` from `(userInfo[...] as? NSNumber)?.uintValue`.
