...
- [x] **[TEST-03] P0 — Add a network seam and URLProtocol-based mock Subsonic server.** `AsyncAPILoader.swift:41-49` uses a fileprivate module-global ephemeral URLSession. Every loader, StreamHandler, and Jukebox needs to run against canned responses.
...
- [x] **[TEST-04] P1 — Protocol-based dependency injection for core singletons.** `DependencyInjection.swift` registers 10 concrete singletons (Store, SavedSettings, BassPlayer, DownloadsManager, DownloadQueue, StreamManager, Jukebox, PlayQueue, Social, Analytics); nothing is protocol-abstracted, so consumers can't get fakes.
...
- [x] **[TEST-05] P1 — Make SavedSettings testable via injected UserDefaults.** `SavedSettings.swift:25` hardcodes `UserDefaults.standard`, including the `@UserDefault` property wrapper.
...
- [x] **[TEST-06] P0 — Create the XCUITest target, launch-argument test mode, and accessibility identifiers.** No UI-test target exists, no launch-argument handling anywhere, and no view sets an `accessibilityIdentifier`, so UI tests can't force a known state or address elements reliably.
...
- [x] **[COV-02] P1 — Pure utility & math tests (no refactor needed).** Foundation extensions, formatters, audio math, stream thresholds, error mapping.
...
- [x] **[COV-03] P1 — Data Store CRUD suite for all 15 store extensions.** Include regression assertions for the known SQL bugs (BUG-06…09) so the suite fails until they're fixed.
...
- [x] **[COV-06] P1 — URLRequest+Subsonic request-construction tests.** Version selection, auth, GET/POST, ranges, encoding — all directly affect server compatibility.
...
- [x] **[COV-07] P1 — Async loader integration tests (all ~22 loaders, incl. error paths).**
...
- [x] **[COV-08] P1 — Download/stream state-machine tests.** DownloadQueue, StreamManager, StreamHandler — this is where most self-flagged "logic is wrong" TODOs live.
...
- [x] **[COV-11] P2 — Audio-engine integration tests against real BASS.** Drive Bass/BassPlayer/BassEqualizer with bundled fixture audio.
  > **Note (found while writing this suite, since fixed):** `BassPlayer.handleInterruption`/`handleRouteChange` decoded `notification.userInfo` values with `as? AVAudioSession.InterruptionType` etc., which fails for the `NSNumber` raw values the system actually sends — so real audio-session interruptions and route changes were ignored. Fixed by decoding via `init(rawValue:)`; the audio-session tests in `BassAudioEngineTests` now assert the pause/resume behavior directly.
...
- [x] **[COV-12] P2 — Singleton/lifecycle integration tests.** Jukebox parsing, online/offline transitions, ServerChecker.
...
- [x] **[COV-13] P2 — View-controller logic unit tests (batch).** Pure-ish logic currently buried in VCs.
...
- [x] **[E2E-02] P1 — Online-mode regression suite (per tab).**
...
- [x] **[E2E-03] P1 — Offline-mode suite.**
  > **Note (found while writing this suite):** a cold `-MODE offline` launch never shows the "iSub is Offline" indicator banner — nothing posts `didEnterOfflineMode` when the mode is set before the UI loads (documented in-test with a non-strict `XCTExpectFailure`). Harness fix that landed with this suite: `UITestSupport` re-seeded the test server with a different URL on every launch, which broke downloaded file paths (keyed by `Server.path`) across relaunches; it now keeps the existing URL when not running the mock server.
...
- [x] **[E2E-04] P2 — Jukebox-mode suite.**
...
- [x] **[BUG-04] P0 — Download bandwidth throttling is never applied.** `StreamHandler.swift:431-456` — the throttle helpers exist but nothing calls them; `urlSession(_:dataTask:didReceive:)` only updates a `throttlingDate` that is never used. Background caching competes with the playing stream and cellular limits.
...
- [x] **[BUG-05] P0 — `DownloadQueue.stop()` has an inverted guard — can't stop an active download.** `DownloadQueue.swift:121` — `guard !isDownloading else { return }` returns exactly when a download IS in progress. `removeCurrentSong()`, `clear()`, and offline-mode entry all fail to cancel the in-flight handler.
...
- [x] **[BUG-11] P1 — Inverted bounds guard in `BassEqualizer.removeEqualizerValue` — deleting an EQ band is a no-op (or crash).** `BassEqualizer.swift:148` — `guard value.arrayIndex >= eqValues.count else { return }` returns for every valid index and proceeds (to an out-of-bounds `remove(at:)`) only for invalid ones.
...
- [x] **[BUG-12] P1 — Landscape visualizer swipe gestures target the class, not the instance.** `EqualizerViewController.swift:43-44` — recognizers are created with `target: EqualizerViewController.self`, so fullscreen-landscape swipes to change visualizer type never fire.
...
- [x] **[BUG-13] P1 — Debug leftover auto-prompts "save preset" every time the EQ opens.** `EqualizerViewController.swift:241-243` — `viewDidAppear` unconditionally schedules `promptToSaveCustomPreset()` after 2 s; no counterpart in the shipping app.
...
- [x] **[BUG-15] P1 — HTTP 5xx during streaming may not propagate as a failure.** `StreamHandler.swift:283` — the 5xx branch cancels the task but the `didFailWithError` call is commented out with a TODO; a 500 may be treated as a silent normal completion.
...
- [x] **[BUG-16] P1 — Cache eviction can loop forever, and runs on the main thread.** `DownloadsManager.swift:132` — `removeOldestCachedSongs` has no break when nothing more can be deleted; `checkCache`/`findCacheSize` (full recursive FS walk) run on the main queue every 60 s.
...
- [x] **[BUG-17] P1 — Wrong password on first server add leaves a phantom/duplicate entry.** `ServerEditViewController.swift:13` — file-header bug note; the code only calls `store.add` on success, so the extra entry comes from somewhere latent (legacy `servers` UserDefaults key, seed row, or nextServerId logic).
...
- [x] **[BUG-18] P1 — GRDB reentrancy: nested pool access inside write transactions.** *(Fixed: the nested id lookups now take the in-transaction `db` handle, the song/file lookups moved ahead of the write transactions, and the reentrancy turned out to be an uncatchable GRDB precondition trap — a guaranteed crash, not just a risk. Regression tests in BookmarkStoreTests and DownloadsStoreTests.)* `BookmarkStore.addBookmark` calls `nextLocalPlaylistId`/`nextBookmarkId` (each opens `pool.read`) inside `pool.write`; `DownloadsStore.deleteDownloadedSongs(serverId:level:/downloadedTagArtist:/downloadedTagAlbum:)` call `self.song(...)` inside their writes. DatabasePool access is not reentrant — deadlock/throw risk.
...
- [x] **[BUG-19] P1 — Wrong column affinities for text songId/path columns.** `DownloadedSong.path`, `DownloadedSongPathComponent.songId`, `serverPlaylistSong.songId`, and `tagSongList.songId` are declared `.integer` but hold text; non-numeric Subsonic IDs risk lossy coercion and broken joins against the text `song.id`.
...
- [x] **[BUG-31] P1 — Jukebox status songs are queued without persisting their metadata, breaking the jukebox queue.** *(Found by E2E-04.)* `Jukebox.swift` `queueDataTask` — a `jukeboxPlaylist` response clears the local jukebox queue and re-queues `Song(serverId:element:)` objects via `song.queue()`, which only inserts `(serverId, songId)` into `localPlaylistSong`; the song metadata is never saved to the `song` table. Every JOIN-based read (`LocalPlaylist.fetchSong`/`fetchSongs`) then returns nothing: queue rows render blank, `PlayQueue.playSong` finds no `currentSong` and never issues the jukebox skip. Since `getInfo` runs 0.5 s after *every* jukebox call, this wipes the freshly-queued local playlist right after play-all/shuffle, so starting jukebox playback is broken end-to-end (the old app persisted the song when adding it to the playlist table).
  > **Prompt:** In Jukebox.swift's queueDataTask response handling (and anywhere else that queues parsed-from-XML songs), persist the Song records before/while queueing (e.g. a store.queue(song:) variant that upserts the song row in the same transaction). Reconcile the get-response queue replacement with locally queued songs so replacePlaylistWithLocal + skip flows survive the 0.5 s getInfo refresh. Add a unit test feeding a jukeboxPlaylist response for never-browsed songs and asserting playQueue.songs()/currentSong resolve, plus flip the E2E-04 XCTExpectFailure assertions (play-all skip, queue-row skip).
...
- [x] **[BUG-22] P2 — `DownloadQueueViewController.viewWillAppear` never calls super.** The base `AbstractDownloadsViewController.viewWillAppear` performs `registerForNotifications()` and `reloadTable()`, so the queue tab doesn't refresh on appear.
...
- [x] **[BUG-23] P2 — Lock-screen next/prev report `.commandFailed` on success; jukebox seek ignored.** `LockScreenAudioControls.swift` — nextTrack/previousTrack handlers return `.commandFailed` after succeeding; changePlaybackPositionCommand only drives the local player, so lock-screen scrubbing in Jukebox mode does nothing.
...
- [x] **[BUG-24] P2 — Force-offline / cellular-disabled settings ignored at launch.** `SceneDelegate.sceneDidBecomeActive` only checks reachability; the old app also entered offline mode for `isForceOfflineMode` and cellular+`isDisableUsageOver3G`, with explanatory alerts.
...
- [x] **[STUB-01] P0 — Saving a playlist to the server is not implemented anywhere.** `createPlaylist` is missing from the `SubsonicAction` enum; all three call sites are stubs: `PlayQueueViewController.uploadPlaylist` (~217), `LocalPlaylistViewController.uploadPlaylist` (~51), `LocalPlaylistsViewController` (~87/~179 incl. the exists→overwrite check and cancel support).
  > **Note (implementation):** all three call sites now share `SavePlaylistFlow` (location prompt, name prompt, server upload with overwrite confirmation, cancel, error alerts) backed by the new `AsyncServerPlaylistCreateLoader`. Verified against the real Airsonic API that `createPlaylist` with a `name` always creates a duplicate even when the name exists, so the overwrite path re-fetches the server playlists and sends `playlistId` instead of `name`. The local-save branch keeps its pre-existing behavior; its overwrite check + error handling is STUB-10.
...
- [x] **[STUB-02] P0 — Deleting server playlists does nothing.** `ServerPlaylistsViewController.deleteServerPlaylists` (~102) is empty; `deletePlaylist` is not in `SubsonicAction`. Handles both swipe-delete and the edit-header delete.
  > **Note (implementation):** added `AsyncServerPlaylistDeleteLoader`, per-selection delete with HUD/cancel/error alert (store rows removed only after the server confirms), and select-all-when-nothing-selected. Also fixed `reloadData()` to manage the edit header, which previously never appeared on the tab's first visit (it was only added when playlists were already cached). E2E swipe-delete assertion flipped strict; new select-all E2E case added (asserting on hittability — deleted rows linger invisibly in the table's reuse pool, so `.exists` is the wrong check).
...
- [x] **[STUB-03] P0 — Deleting local playlists does nothing.** `LocalPlaylistsViewController.deleteLocalPlaylists` (~142) is an empty stub used by both swipe-delete and the edit header; select-all-then-delete is also missing.
...
- [x] **[STUB-05] P0 — Bookmark batch delete (edit mode) does nothing.** `BookmarksViewController.saveEditHeaderSaveDeleteAction` (~98) is fully commented out (and references the wrong method); only single swipe-delete works.
  > **Note (implementation):** batch delete via the existing `Store.delete(bookmark:)` cascade (snapshot playlist included, covered by `testDeleteBookmarkCascadesSnapshotPlaylist`), with select-all when nothing is selected (clear-all flow). E2E clear-all flipped strict with the two-tap select-all-then-delete flow.
...
- [x] **[STUB-06] P0 — iCloud/iTunes backup exclusion for downloads is a no-op.** `DownloadsManager.swift:204-214` — both setAllCachedSongsTo(Not)Backup bodies are commented out, so the `isBackupCacheEnabled` toggle is inert; StreamHandler also never sets the flag on new files.
  > **Note (implementation):** per-file enumeration on the cache-check background queue (the flag doesn't propagate from directories), plus StreamHandler stamps `isExcludedFromBackup = !isBackupCacheEnabled` on every freshly created download file. Tests cover the enumerator, the settings-toggle wiring, and the new-file stamp.
...
- [x] **[STUB-07] P1 — No user alert when downloads stop for low disk space.** `DownloadQueue.swift:55-58` — silent return at ≤25 MB free; the old showNoFreeSpaceMessage UI is commented out.
  > **Note (implementation):** `DownloadsManager.showNoFreeSpaceMessage()` (gated on `isPopupsEnabled`, main thread) restores the old CacheSingleton messaging; `DownloadQueue.start()` calls it on the low-space halt. `DownloadsManager` is no longer `final` so tests can stub `freeSpace` and observe the alert.
