...
| `-FIXTURES <name>` | Selects a named fixture response set. `default` maps every supported Subsonic action to the captured Airsonic responses in `Tests/iSubTests/Fixtures/XML/` (bundled into beta builds). `badauth` overrides `ping` with the wrong-credentials error for failed-auth flows. Add new sets in `UITestURLProtocol.fixtureSets`. |
