...
- (void)editPlaylistAction:(id)sender {
	if (self.isEditing) {
        ...
        if (PlayQueue.shared.currentIndex >= 0 && PlayQueue.shared.currentIndex < self.currentPlaylistCount) {
            [self.tableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:PlayQueue.shared.currentIndex inSection:0] animated:NO scrollPosition:UITableViewScrollPositionTop];
        }
    } else {
...
- (void)savePlaylistAction:(id)sender {
	if (self.deleteSongsLabel.hidden == YES) {
        ...
        if (selectedRowIndexes.count == 0) {
			...
			// Delete action
            [PlayQueue.shared removeSongsWithIndexes:selectedRowIndexes];
			[self updateCurrentPlaylistCount];
...
//			[self.tableView reloadData];
		...
		NSUInteger songCount = PlayQueue.shared.count;
        if (songCount == 1) {
			self.playlistCountLabel.text = [NSString stringWithFormat:@"1 song"];
        } else {
...
//	if (fromIndexPath.row == PlayQueue.shared.currentIndex) {
//		PlayQueue.shared.currentIndex = toIndexPath.row;
//	} else {
//		if (fromIndexPath.row < PlayQueue.shared.currentIndex && toIndexPath.row >= PlayQueue.shared.currentIndex) {
//			PlayQueue.shared.currentIndex = PlayQueue.shared.currentIndex - 1;
//		} else if (fromIndexPath.row > PlayQueue.shared.currentIndex && toIndexPath.row <= PlayQueue.shared.currentIndex) {
//			PlayQueue.shared.currentIndex = PlayQueue.shared.currentIndex + 1;
//		}
//	}
//	
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!song.isVideo) {
        return [SwipeAction downloadQueueAndDeleteConfigWithModel:song deleteHandler:^{
            [PlayQueue.shared removeSongsWithIndexes:@[@(indexPath.row)]];
            [self updateCurrentPlaylistCount];
