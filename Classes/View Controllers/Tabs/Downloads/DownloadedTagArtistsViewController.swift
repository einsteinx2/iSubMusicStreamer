...
final class DownloadedTagArtistsViewController: AbstractDownloadsViewController {
    ...
    override func reloadTable() {
        downloadedTagArtists = settings.isCombinedContext ? store.downloadedTagArtists() : store.downloadedTagArtists(serverId: serverId)
        super.reloadTable()
...
extension DownloadedTagArtistsViewController {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let model = downloadedTagArtists[indexPath.row]
