...
@objc final class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UIDevice.isPad() {
            ...
            let foldersTab = CustomUINavigationController(rootViewController: FolderArtistsViewController())
            foldersTab.tabBarItem = UITabBarItem(title: "Folders", image: UIImage(named: "tabbaricon-folders"), tag: 0)
            self.foldersTab = foldersTab
            
            let artistsTab = CustomUINavigationController(rootViewController: ArtistsViewController())
